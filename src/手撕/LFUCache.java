...
class LFUCache {
    ...
    private final Map<Integer, Node> freqToDummy = new HashMap<>(); // ✅频率 - 头节点
    private int minFreq;// ✅当前 min
    ...
    public void put(int key, int value) {
        ...
        if (keyToNode.size() == capacity) { // 书太多了
            ...
            if (dummy.prev == dummy) { // 移动完后 这摞书是空的
    ...
    private Node getNode(int key) {
        ...
        remove(node); // 把这本书抽出来
        Node dummy = freqToDummy.get(node.freq); // node.freq
        if (dummy.prev == dummy) { // 抽出来后，这摞书是空的
            freqToDummy.remove(node.freq); // ✅ 链表删了
            if (minFreq == node.freq) minFreq++; // ✅这摞书是最左边的
        }
        node.freq++; // 看书次数 +1
